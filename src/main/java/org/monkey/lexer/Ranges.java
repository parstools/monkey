...
public class Ranges {
    ...
    public Ranges(String rangeStr, boolean invert) throws Exception {
        ...
        if (invert) invertSet();
        getRanges();
        System.out.println(rangesFrom);
        System.out.println(rangesTo);
    }
    ...
    private static char slashed(char c, String s) throws Exception {
        switch (c) {
            ...
            case '-': return '-';
            case ']': return ']';
            default: throw new Exception("range definition "+s+ " is unknown \\"+c);
        }
    }

    private char readChar(String s) throws Exception {
        char c = s.charAt(pos);
        if (c==']') throw new Exception("range definition "+s+ " is ] without \\");
        else if (c=='\t') throw new Exception("range definition "+s+ " for tab use \\t");
        else if (c=='\\') {
            pos++;
            if (pos>=s.length()-1) throw new Exception("range definition "+s+ " ends with \\");
            char nextc = s.charAt(pos);
            return slashed(nextc,s);
        }
        else if (c=='-') throw new Exception("range definition "+s+ " is - without \\");
        else return c;
    }
    ...
    private void parseRanges(String s) throws Exception {
        if (s.isEmpty() || s.charAt(0)!='[' || s.charAt(s.length()-1)!=']')
            throw new Exception("Bad range definition "+s);
        pos=1;
        int from, to;
